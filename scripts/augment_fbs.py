...
GENERIC_TEXT = {
    "房市焦點", "市場趨勢", "政策新聞", "國際地產", "都更危老", "金融理財",
    "建案開箱", "私房觀點", "地產科技", "房產知識", "富比士地產王", "首頁",
    "更多", "看更多", "最新動態", "重點新聞", "上一篇", "下一篇",
}
...
def canonical_story_url(href: str, base_url: str) -> str | None:
    ...
    if not re.match(r"^/news/\d+(?:-[^/?#]+)?$", path):
        return None
    return f"https://www.fbs168.com{path}"
...
def fetch_article_details(url: str, title: str) -> tuple[str, datetime | None, str]:
    try:
        ...
        paragraphs: list[str] = []
        for selector in (
            "article p", ".article-content p", ".article-body p", ".content p", "main p",
        ):
            for p in soup.select(selector):
                text = p.get_text(" ", strip=True)
                if (
                    len(text) >= 25
                    and "富比士地產王" not in text[:20]
                    and "訂閱富比士" not in text
                ):
...
def main() -> None:
    ...
    print(f"[fbs-source] {FEED['name']} {FEED['url']}")
    try:
        html = fetch(FEED["url"])
        candidates = extract_candidates(html, FEED["url"])
        candidates = candidates[: int(FEED.get("max_candidates", 40))]
        print(f"[fbs-candidates] {len(candidates)}")
    except Exception as exc:
        print(f"[fbs-source-error] {exc}")
        candidates = []
    ...
    print(f"[fbs-result] added {added}, housing {before} -> {after} after dedupe")
